          ...
          <div className="pt-3 mt-2 border-t border-slate-100">
            ...
            {peopleToShow.map(person => (
              <div key={person._id} onClick={() => onStartNewChat(person._id)} className="flex items-center gap-3 p-2.5 rounded-2xl hover:bg-slate-50 active:bg-slate-100 cursor-pointer transition-colors">
                <div className="relative shrink-0">
                  <img src={person.profileImage || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150"} alt={`${person.firstName} ${person.lastName}`} className="w-10 h-10 rounded-xl object-cover" />
                  {person.isOnline && <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-500 border-2 border-white" />}
                </div>
                <div className="flex-1 min-w-0">
